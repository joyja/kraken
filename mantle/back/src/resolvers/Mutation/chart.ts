...
import { 
  ...
  ChartPage as ResolverChartPage, 
  CreateChartEntry, 
  UpdateChartEntry,
  DeleteChartEntry,
  Chart as ResolverChart,
  CreatePenEntry,
  UpdatePenEntry, 
  DeletePenEntry,
  Pen as ResolverPen 
} from '../../resolvers/types'
import { ChartPage, Chart, Pen } from '@prisma/client'
import { Log } from '../../log'

const log = new Log('Charts')

export async function createChartPage (_root:unknown, { input  }:{ input: CreateChartPageEntry }):Promise<ChartPage> {
  return prisma.chartPage.create({ data: { 
    name: input.name,
    description: input.description,
  }, include: { charts: { include: { pens: true }}}})
}

export async function updateChartPage (_root:unknown, { input }:{ input: UpdateChartPageEntry }):Promise<ChartPage> {
  ...
  return prisma.chartPage.update({ where: { id }, data, include: { charts: { include: { pens: true }}}})
}

export async function deleteChartPage (_root:unknown,{ input:{ id } }:{ input:DeleteChartPageEntry}):Promise<ChartPage> {
  return prisma.chartPage.delete({ where: { id }, include: { charts: { include: { pens: true }}}})
}
...
export async function deleteChart(_root:unknown, {input:{id}}:{input:DeleteChartEntry}):Promise<Chart> {
  return prisma.chart.delete({ where: { id }, include: { pens: true }})
}

export async function createPen(_root:unknown, { input }:{ input:CreatePenEntry }):Promise<ResolverPen> {
  return prisma.pen.create({ data: input })
}

export async function updatePen(_root:unknown, { input }:{ input:UpdatePenEntry }):Promise<ResolverPen> {
  const { id, ...updateFields }:{ id: string; [key: string]: any } = input
  const data:{[key:string]:any} = {}
  // Filter out null values
  Object.keys(updateFields).forEach((key:string) => {
...
export async function deletePen(_root:unknown, {input:{id}}:{input:DeletePenEntry}):Promise<ResolverPen> {
