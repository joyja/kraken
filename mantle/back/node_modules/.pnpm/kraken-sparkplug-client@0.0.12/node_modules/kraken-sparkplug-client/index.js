...
var SparkplugClient = /** @class */ (function (_super) {
    ...
    SparkplugClient.prototype.init = function () {
        ...
        this.client.on('connect', function () {
            ...
            _this.client.subscribe('STATE/#', { qos: 1 });
        ...
        this.client.on('message', function (topic, message) {
            var payload, timestamp, splitTopic, metrics;
            splitTopic = topic.split("/");
            if (splitTopic[0] === _this.version) {
                payload = _this.maybeDecompressPayload(_this.decodePayload(message));
 ...
 * Sparkplug Host
 */
var SparkplugHost = /** @class */ (function (_super) {
    __extends(SparkplugHost, _super);
    function SparkplugHost(config) {
        var _this = _super.call(this) || this;
        // Constants
        _this.type_int32 = 7;
        _this.type_boolean = 11;
        ...
        // Client connection options
        _this.serverUrl = getRequiredProperty(config, "serverUrl");
        _this.primaryHostId = getRequiredProperty(config, 'primaryHostId');
        var username = getRequiredProperty(config, "username");
        var password = getRequiredProperty(config, "password");
        var keepalive = getProperty(config, "keepalive", 5);
        var clientId = getRequiredProperty(config, "clientId");
        _this.mqttOptions = __assign(__assign({}, config.mqttOptions || {}), { clean: true, clientId: clientId, keepalive: keepalive, reschedulePings: false, connectTimeout: 30000, username: username, password: password, will: {
                topic: "STATE/".concat(_this.primaryHostId),
                payload: "OFFLINE",
                qos: 0,
                retain: true,
            } });
    ...
    // Increments a sequence number
    SparkplugHost.prototype.incrementSeqNum = function () {
        if (this.seq == 256) {
            this.seq = 0;
        }
        return this.seq++;
    };
    SparkplugHost.prototype.encodePayload = function (payload) {
        return sparkplugbpayload.encodePayload(payload);
    };
    ;
    SparkplugHost.prototype.decodePayload = function (payload) {
        return sparkplugbpayload.decodePayload(payload);
    };
    SparkplugHost.prototype.addSeqNumber = function (payload) {
        payload.seq = this.incrementSeqNum();
    };
    // Get DEATH payload
    SparkplugHost.prototype.getDeathPayload = function () {
        return {
            "timestamp": new Date().getTime(),
            "metrics": [{
                    "name": "bdSeq",
    ...
    // Logs a message alert to the console
    SparkplugHost.prototype.messageAlert = function (alert, topic, payload) {
        logger.debug("Message " + alert);
        logger.debug(" topic: " + topic);
        logger.debug(" payload: " + JSON.stringify(payload));
    };
    SparkplugHost.prototype.compressPayload = function (payload, options) {
        var algorithm = null, compressedPayload, resultPayload = {
            "uuid": compressed,
            "metrics": []
        };
    ...
    SparkplugHost.prototype.decompressPayload = function (payload) {
        var metrics = payload.metrics || [], algorithm = null;
        var body = payload.body || new Uint8Array();
        logger.debug("Decompressing payload");
        var algorithmMetric = metrics.find(function (m) { return m.name === 'algorithm'; });
    ...
    SparkplugHost.prototype.maybeCompressPayload = function (payload, options) {
        if (options === null || options === void 0 ? void 0 : options.compress) {
            // Compress the payload
            return this.compressPayload(this.encodePayload(payload), options);
        }
    ...
    SparkplugHost.prototype.maybeDecompressPayload = function (payload) {
        if (payload.uuid !== undefined && payload.uuid === compressed) {
            // Decompress the payload
            return this.decodePayload(this.decompressPayload(payload));
        }
    ...
    SparkplugHost.prototype.subscribeTopic = function (topic, options, callback) {
        if (options === void 0) { options = { "qos": 0 }; }
        logger.info("Subscribing to topic:", topic);
        this.client.subscribe(topic, options, callback);
    };
    SparkplugHost.prototype.unsubscribeTopic = function (topic, options, callback) {
        logger.info("Unsubscribing topic:", topic);
        this.client.unsubscribe(topic, options, callback);
    };
    SparkplugHost.prototype.publishHostOnline = function () {
        var _a;
        var topic = "STATE/".concat(this.primaryHostId);
        var payload = 'ONLINE';
        logger.info('Publishing Primary Host Online.');
        (_a = this.client) === null || _a === void 0 ? void 0 : _a.publish(topic, payload, { retain: true });
        this.messageAlert('published', topic, payload);
    };
    SparkplugHost.prototype.publishHostOffline = function () {
        var _a;
        var topic = "STATE/".concat(this.primaryHostId);
        var payload = 'OFFLINE';
    ...
    // Publishes Node Data messages for the edge node
    SparkplugHost.prototype.publishNodeCommand = function (groupId, nodeId, payload, options) {
        var topic = this.version + "/" + groupId + "/NCMD/" + nodeId;
        // Add seq number
        this.addSeqNumber(payload);
        // Publish
    ...
    // Publishes Node Data messages for the edge node
    SparkplugHost.prototype.publishDeviceCommand = function (groupId, nodeId, deviceId, payload, options) {
        var topic = this.version + "/" + groupId + "/DCMD/" + nodeId + "/" + deviceId;
        // Add seq number
        this.addSeqNumber(payload);
        // Publish
    ...
    SparkplugHost.prototype.stop = function () {
        var _a;
        this.publishHostOffline();
        (_a = this.client) === null || _a === void 0 ? void 0 : _a.end();
    };
    // Configures and connects the client
    SparkplugHost.prototype.init = function () {
        var _this = this;
        // Connect to the MQTT server
        this.connecting = true;
        logger.debug("Attempting to connect: " + this.serverUrl);
        ...
        this.client.on('connect', function () {
            ...
            logger.info("Subscribing to control/command messages for both the edge node and the attached devices");
            // Subscribe to state messages
            _this.client.subscribe("".concat(_this.version, "/#"), { qos: 1 });
            // Emit the "birth" event to notify the application to send a births
            _this.emit("birth");
        });
        /*
        ...
        this.client.on('message', function (topic, message) {
            ...
            if (splitTopic[0] === _this.version) {
                ...
                timestamp = payload.timestamp;
                if (splitTopic[0] === _this.version && splitTopic[2] === 'DDATA') {
                    _this.emit('ddata', topic, splitTopic[1], //groupId
                    splitTopic[3], //nodeId
                    splitTopic[4], //deviceId
                    payload);
                }
                else if (splitTopic[0] === _this.version && splitTopic[2] === 'NBIRTH') {
                    _this.emit('nbirth', topic, splitTopic[1], //groupdId
                    splitTopic[3], //nodeId
                    payload);
                }
                else if (splitTopic[0] === _this.version && splitTopic[2] === 'DBIRTH') {
                    _this.emit('dbirth', topic, splitTopic[1], //groupId
                    splitTopic[3], //nodeId
                    splitTopic[4], //deviceId
                    payload);
                }
                else if (splitTopic[0] === _this.version && splitTopic[2] === 'NDEATH') {
                    _this.emit('ndeath', topic, splitTopic[1], //groupId
                    splitTopic[3], //nodeId
                    payload);
                }
                else if (splitTopic[0] === _this.version && splitTopic[2] === 'DDEATH') {
                    _this.emit('ddeath', topic, splitTopic[1], //groupId
                    splitTopic[3], //nodeId
                    splitTopic[4], //deviceId
                    payload);
                }
                else if (splitTopic[0] === _this.version && splitTopic[2] === "NCMD") {
                    // Emit the "command" event
                    _this.emit("ncmd", payload);
                }
                else if (splitTopic[0] === _this.version && splitTopic[2] === "DCMD") {
                    // Emit the "command" event for the given deviceId
                    _this.emit("dcmd", splitTopic[4], payload);
                }
            }
                ...
                if (splitTopic[0] === 'STATE') {
                    _this.emit("state", splitTopic[1], payload);
                }
                else {
                    _this.emit("message", topic, payload);
                }
