...
export class Modbus {
    ...
    if (this.connected) {
      if (registerType === 'INPUT_REGISTER') {
        ...
        const data = await this.client.readInputRegisters(
          ...
          .then((data) => {
            return this.formatValue(data.data, format)
          })
          .catch(async (error) => {
            if (
        ...
        const quantity = 1
        return this.client.readCoils(register, quantity)
          .then((data) => {
            return data.data[0], format
          })
          .catch(async (error) => {
            if (error.name === 'TransactionTimedOutError') {
              await this.disconnect()
  ...
  async write({ value, registerType, register, format }: WriteOptions): Promise<void> {
    if (this.connected) {
