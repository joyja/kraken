...
const dataTypes_1 = require("./enip/cip/dataTypes");
...
class Tag extends events_1.EventEmitter {
    ...
    constructor(tagname, program = null, datatype = null, keepAlive = 0) {
        ...
        if (datatype !== null && !(0, dataTypes_1.isValidTypeCode)(datatype))
        ...
        const isBitIndex = memArr.length > 1 && Number(memArr[memArr.length - 1]) % 1 === 0;
        // Check if BIT_STRING data type was passed in
        const isBitString = datatype === dataTypes_1.Types.BIT_STRING &&
            Number(pathArr[pathArr.length - 1]) % 1 === 0;
     ...
     */
    get type() {
        return (0, dataTypes_1.getTypeCodeString)(this.state.tag.type);
    }
    /**
     * Gets Tag Bit Index
     ...
     */
    set type(type) {
        if (!(0, dataTypes_1.isValidTypeCode)(type))
            throw new Error('Datatype must be a Valid Type Code <number>');
    ...
    parseReadMessageResponseValueForBitIndex(data) {
        const { tag } = this.state;
        const { SINT, INT, DINT, BIT_STRING } = dataTypes_1.Types;
        // Read Tag Value
    ...
    parseReadMessageResponseValueForAtomic(data) {
        const { SINT, INT, DINT, REAL, BOOL } = dataTypes_1.Types;
        // Read Tag Value
        /* eslint-disable indent */
        switch (this.state.tag.type) {
            case SINT:
    ...
    static isValidTagname(tagname) {
        ...
        if (!userRegex.test(tagname) && !moduleRegex.test(tagname))
            return false;
        // check segments
        if (tagname.split(/[:.[\],]/).filter((segment) => segment.length > 40)
            .length > 0)
