...
class TagList {
    ...
    _generateListMessageRequest(instanceID = 0, program) {
        const { LOGICAL, DATA } = enip_1.CIP.EPATH.segments;
        let pathArray = [];
        if (program) {
        ...
        const request = enip_1.CIP.MessageRouter.build(enip_1.CIP.MessageRouter.services.GET_INSTANCE_ATTRIBUTE_LIST, Buffer.concat(pathArray), requestData);
        return request;
    }
    /**
    ...
    _getTagTypeNames() {
        for (const tag of this.tags) {
            tag.type.typeName = enip_1.CIP.DataTypes.getTypeCodeString(tag.type.code);
