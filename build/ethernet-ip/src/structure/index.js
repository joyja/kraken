...
exports.Template = exports.Structure = void 0;
...
class Structure extends tag_1.default {
    /**
    ...
    constructor(tagname, taglist, program = null, datatype = null, keepAlive = 0, arrayDims = 0, arraySize = 0x01) {
        ...
        if (this._template)
            super.type = enip_1.CIP.DataTypes.Types.STRUCT;
    }
    /**
    ...
    generateWriteMessageRequest(value = null, size = 0x01) {
        const { STRUCT } = enip_1.CIP.DataTypes.Types;
        if (!this._template) {
    ...
    _parseReadData(data, template) {
        if (template._members.length === 2 && template._members[0].name === "LEN" && template._members[1].name === "DATA")
            return (0, utilities_1.bufferToString)(data);
        let structValues = {};
        const { SINT, INT, DINT, REAL, LINT, BIT_STRING, BOOL, STRUCT } = enip_1.CIP.DataTypes.Types;
    ...
    _parseWriteData(structValues, template) {
        if (template._members.length === 2 && template._members[0].name === "LEN" && template._members[1].name === "DATA")
            return (0, utilities_1.stringToBuffer)(structValues, template._attributes.StructureSize);
                    ...
                    if (member.type.arrayDims > 0) {
                        for (let i = 0; i < member.info; i++) {
                            ...
                            for (let j = i * 32; j < (i + 1) * 32; j++) {
                                if (j > structValues[member.name].length)
                                    break;
