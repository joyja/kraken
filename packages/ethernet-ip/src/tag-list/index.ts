...
class TagList {
  ...
  getControllerTags(
    ...
    return new Promise((resolve, reject) => {
      const getListAt = (instanceID = 0) => {
        // Create function that we can call back in recursion
  ...
  getTag(tagName: string, program: string = null): tagListTag {
    return this.tags.find(
      (tag) => tag.name === tagName && tag.program === program,
    )
  }
  ...
  getTemplateByTag(tagName: string, program: string = null): Template {
    ...
    if (tag) {
      ...
      while (finalTemplate && tagArrayPointer < tagArray.length) {
        ...
        const nextTag = finalTemplate._members.find(
      ...
      for (const tag of this.tags) {
        if (tag.type.structure && !this.templates[tag.type.code]) {
          try {
            ...
            /* ignore template fetching errors */
          }
        }
      }
