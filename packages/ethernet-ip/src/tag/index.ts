...
const { READ_TAG, WRITE_TAG, READ_MODIFY_WRITE_TAG, READ_TAG_FRAGMENTED } =
  MessageRouter.services
import {
  Types,
  getTypeCodeString,
  isValidTypeCode,
} from '../enip/cip/data-types'
...
export class Tag extends EventEmitter {
  ...
  constructor(
    ...
    let isBitIndex =
      memArr.length > 1 && !isNaN(Number(memArr[memArr.length - 1]))
  ...
  set read_size(size: number) {
    if (typeof size !== 'number') throw new Error('Read Size must be a number')
