  ...
  describe('SendRRDataReceived Handler', () => {
    it('Forward Open', () => {
      ...
      const srrdBuf = Buffer.from([
        212, 0, 0, 0, 65, 2, 188, 0, 34, 34, 34, 34, 66, 66, 51, 51, 55, 19, 0,
        0, 16, 39, 0, 0, 16, 39, 0, 0, 0, 0,
      ])
      const srrd = [
      ...
      const retBuf = Buffer.from([
        65, 2, 188, 0, 34, 34, 34, 34, 66, 66, 51, 51, 55, 19, 0, 0, 16, 39, 0,
        0, 16, 39, 0, 0, 0, 0,
      ])
    ...
    it('Forward Close', () => {
      ...
      jest.spyOn(plc, 'emit')
      const srrdBuf = Buffer.from([
        206, 0, 0, 0, 66, 66, 51, 51, 55, 19, 0, 0, 0, 0,
      ])
      const srrd = [
        { TypeID: 0, data: Buffer.from([]) },
        { TypeID: 178, data: srrdBuf },
    ...
    it('Multiple Service Packet', () => {
      ...
      jest.spyOn(plc, 'emit')
      const srrdBuf = Buffer.from([
        138, 0, 0, 0, 2, 0, 6, 0, 14, 0, 204, 0, 0, 0, 195, 0, 241, 216, 204, 0,
        0, 0, 195, 0, 64, 34,
      ])
      const srrd = [
  ...
  describe('SendUnitDataReceived Handler', () => {
    it('Get Attribute All', () => {
      ...
      const sudBuf = Buffer.from([
        1, 0, 129, 0, 0, 0, 1, 0, 14, 0, 77, 0, 17, 4, 112, 32, 232, 2, 61, 64,
        22, 49, 55, 54, 57, 45, 76, 51, 50, 69, 47, 65, 32, 76, 79, 71, 73, 88,
        53, 51, 51, 50, 69,
      ])
      const sud = [
      ...
      const retBuf = Buffer.from([
        1, 0, 14, 0, 77, 0, 17, 4, 112, 32, 232, 2, 61, 64, 22, 49, 55, 54, 57,
        45, 76, 51, 50, 69, 47, 65, 32, 76, 79, 71, 73, 88, 53, 51, 51, 50, 69,
      ])
    ...
    it('Multiple Service Packet', () => {
      ...
      jest.spyOn(plc, 'emit')
      const sudBuf = Buffer.from([
        2, 0, 138, 0, 0, 0, 2, 0, 6, 0, 14, 0, 204, 0, 0, 0, 195, 0, 241, 216,
        204, 0, 0, 0, 195, 0, 64, 34,
      ])
      const sud = [
        { TypeID: 161, data: Buffer.from([34, 34, 34, 34]) },
        { TypeID: 177, data: sudBuf },
