...
class Controller extends ENIP {
  ...
  async _writeTag(
    ...
    if (
      tag.state.tag.value.length > 480 - tag.path.length &&
      tag instanceof Structure
    )
          ...
          this.on('Multiple Service Packet', async (err, data) => {
            ...
            for (let i = 0; i < data.length; i++) {
              if (data[i].generalStatusCode === 6) {
                await this._readTagFragmented(
                  group.state.tags[msg.tag_ids[i]],
                ).catch(reject)
              }
            }
  ...
  async _writeTagGroup(group: TagGroup): Promise<void> {
    ...
    for (let msg of messages) {
      if (msg.data) {
        ...
        await this.writeTag(msg.tag).catch((e) => {
          throw e
        })
      }
    }
  ...
  newTag(
    ...
    if (template) {
      ...
      if (subscribe) this.subscribe(tag)
      return tag
    } else {
  ...
  getTagByName(name: string): Tag | Structure {
    return Object.values(this.state.subs.state.tags).find(
      (tag) => tag.state.tag.name === name,
    )
  }
}
