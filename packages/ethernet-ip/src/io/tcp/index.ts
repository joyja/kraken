...
class Controller extends ENIP {
  ...
  async forwardOpen() {
    ...
    const paramsOT = CIP.ConnectionManager.build_connectionParameters(
      owner.Exclusive,
      connectionType.PointToPoint,
      priority.Scheduled,
      fixedVar.Fixed,
      6 + this.outputInstance.size,
    )
    const paramsTO = CIP.ConnectionManager.build_connectionParameters(
      owner.Exclusive,
      connectionType.PointToPoint,
      priority.Scheduled,
      fixedVar.Fixed,
      2 + this.inputInstance.size,
    )
  ...
  _ioPath() {
    ...
    const electronicKey = Buffer.concat([
      path,
      vendorID,
      deviceType,
      productCode,
    ...
    // Config Instance
    const configInstance = LOGICAL.build(
      LOGICAL.types.InstanceID,
      this.configInstance.assembly,
    )
    // Output Instance
    const pointOT = LOGICAL.build(
      LOGICAL.types.ConnPoint,
      this.outputInstance.assembly,
    )
    // Input Instance
    const pointTO = LOGICAL.build(
      LOGICAL.types.ConnPoint,
      this.inputInstance.assembly,
    )

    const cipPath = Buffer.concat([
      assemblyObjectClass,
      configInstance,
      pointOT,
      pointTO,
  ...
  _handleSendRRDataReceived(srrd) {
    const { service, generalStatusCode, extendedStatus, data } =
    ...
    switch (service - 0x80) {
      ...
      case MULTIPLE_SERVICE_PACKET: {
        ...
        for (let i = 0; i < offsets.length - 1; i++) {
          ...
          if (
            msgData.generalStatusCode !== 0 &&
            error.generalStatusCode !== 30
          ) {
            error = {
