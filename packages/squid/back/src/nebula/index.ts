...
import {
  ...
  NebulaHostCertInput,
  NebulaInstallInput,
  NebulaReceiveCertInput,
  NebulaRequestSendCertInput,
  NebulaSendCertInput
} from './types.js'
import { getDefaultConfig } from './defaultConfigs.js'
import { exec } from 'child_process'
import { getSystemdConfig } from './service.js'
import {
  mqtt,
  MQTTData,
  MqttDataDeviceControl,
  type MqttDataMetric
} from '../mqtt/index.js'
...
class NebulaCert {
  ...
  generateCaCertificate({ name, allowOverwrite }: NebulaCaCertInput) {
    ...
    return new Promise<void>((resolve, reject) => {
      exec(
        `nebula-cert ca -name ${name} -out-crt /etc/squid/nebula/ca.crt -out-key /etc/squid/nebula/ca.key -out-qr /etc/squid/nebula/ca.png`,
        (err, stdout, stderr) => {
          if (
            err ||
            ...
            reject(
              `Error generating ca certificate. Error: ${err ? JSON.stringify(err, null, 2) : 'Missing some ca files.'}`
            )
          } else {
  ...
  generateHostCertificate({
    ...
    return new Promise<void>((resolve, reject) => {
      const certName = isOwn ? 'host' : name
      exec(
        `nebula-cert sign -ca-crt /etc/squid/nebula/ca.crt -ca-key /etc/squid/nebula/ca.key -name ${name} -ip ${nebulaIp} ${groups ? `-groups ${groups.join(',')}` : ''} -out-crt /etc/squid/nebula/${certName}.crt -out-key /etc/squid/nebula/${certName}.key -out-qr /etc/squid/nebula/${certName}.png`,
        (err, stdout, stderr) => {
          if (
            err ||
            ...
            reject(
              `Error generating host certificate. Error: ${err ? JSON.stringify(err, null, 2) : 'Missing some host files.'}`
            )
          } else {
            log.info(
              `Successfully generated host certificate, ${certName}.crt.`
            )
            resolve()
          }
        }
  ...
  requestSendCert({
    ...
    log.info(
      `Requesting certificate for ${name} from ${groupId}/${nodeId}/${deviceId}.`
    )
  ...
  sendCert({
    ...
    log.info(
      `Sending certificate for ${name} to ${requesterGroupId}/${requesterNodeId}/${requesterDeviceId}.`
    )
    return new Promise<void>(async (resolve, reject) => {
      await this.generateHostCertificate({
        isOwn: false,
        name,
        nebulaIp,
        groups
  ...
  get hasHostCertificate() {
    return (
      fs.existsSync(`/etc/squid/nebula/host.crt`) &&
      fs.existsSync(`/etc/squid/nebula/host.key`)
    )
  }
}
...
class Nebula extends MQTTData {
  ...
  async install({
    ...
    if (!this.isInstalled || allowReinstall) {
      const downloadUrl = await this.fetchReleases().then((releases) => {
        return releases
          .find((release) => {
      ...
      if (!downloadUrl) {
        throw new Error(
          `Could not find download url for nebula version ${version}`
        )
      }
      const tarStream = await octokit
        .request(downloadUrl, { request: { parseSuccessResponseBody: false } })
        .then(({ data }) => data)
      await new Promise<void>((resolve, reject) => {
        stream.pipeline(
          tarStream,
          gunzip(),
          tar.extract('/usr/local/bin'),
          (err) => {
            if (err) {
              reject(err)
            } else {
      ...
      if (isLighthouse) {
        ...
        if (
          ...
          throw Error(
            `Need lighthouse group, node, and device ids to request a certificate. Got: ${JSON.stringify({ lighthouseGroupId, lighthouseNodeId, lighthouseDeviceId, nebulaIp, groups, name }, null, 2)}`
          )
        }
      }
    } else {
  ...
  getIsServiceRunning() {
    return runCommand('systemctl is-active --quiet squid-nebula.service')
      .then(() => {
        return true
      })
      .catch((err) => {
  ...
  get isInstalled() {
    return (
      fs.existsSync('/usr/local/bin/nebula') &&
      fs.existsSync('/usr/local/bin/nebula-cert')
    )
  }
  get config() {
