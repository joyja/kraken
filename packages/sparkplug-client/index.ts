...
interface SparkplugClient extends events.EventEmitter {
  ...
  on(
    event: 'state',
    listener: (primaryHost: string, payload: string) => void
  ): this
  /** emitted when a payload is received with a version unsupported by this client */
  on(
    event: 'message',
    listener: (topic: string, payload: UPayload) => void
  ): this
...
class SparkplugClient extends events.EventEmitter {
  ...
  subscribeTopic(
    topic: string,
    options: mqtt.IClientSubscribeOptions = { qos: 0 },
  ...
  publishDeviceCommand(
    ...
    let topic =
      this.version + '/' + groupId + '/DCMD/' + nodeId + '/' + deviceId
    // Add seq number
    this.addSeqNumber(payload)
    // Publish
  ...
  private init() {
    ...
    this.client.on('connect', () => {
      ...
      this.client!.subscribe(
        this.version + '/' + this.groupId + '/NCMD/' + this.edgeNode + '/#',
        { qos: 0 }
      )
      this.client!.subscribe(
        this.version + '/' + this.groupId + '/DCMD/' + this.edgeNode + '/#',
        { qos: 0 }
      )
