...
class UserHandler {
	async create({ input }: { input: ResolverCreateUser }): Promise<ResolverUser> {
		const { id, ...data } = input
		return await prisma.user.create({ data: id !== null && id !== undefined ? input : data }).then(prismaToResolver)
	}

	async update({ input }: { input: ResolverUpdateUser }): Promise<ResolverUser> {
		...
		return await prisma.user.update({ where: { id }, data }).then(prismaToResolver)
	}
	...
	async getAll(): Promise<ResolverUser[]> {
		return await prisma.user.findMany().then((users) => users.map(prismaToResolver))
	}
