...
import { type Roster as PrismaRoster, type RosterEntry, type Alarm } from '@prisma/client'
import { alarmHandler } from './alarm.js'
// import { Log } from './log';

// const log = new Log('Roster');

function prismaToResolver(prismaRoster: PrismaRoster & {alarms:Alarm[]} & {users:RosterEntry[]}): ResolverRoster {
...
export class RosterHandler {
	async create({ input }: { input: ResolverCreateRoster }): Promise<ResolverRoster> {
		return await prisma.roster
			.create({ data: input, include: { users: true, alarms: true } })
			.then(prismaToResolver)
	}

	async update({ input }: { input: ResolverUpdateRoster }): Promise<ResolverRoster> {
	...
	async createEntry({ input }: { input: ResolverCreateRosterEntry }): Promise<ResolverRosterEntry> {
	...
	async updateEntry({ input }: { input: ResolverUpdateRosterEntry }): Promise<ResolverRosterEntry> {
		const { id, ...updateFields }: { id: string; [key: string]: any } = input
		const data: Record<string, any> = {}
		// Filter out null values
		Object.keys(updateFields).forEach((key: string) => {
	...
	async getActiveRosters(): Promise<ResolverRoster[]> {
		...
		return rosters
			.map((roster) => {
				return roster !== null && roster !== undefined ? prismaToResolver(roster) : null
			}) as ResolverRoster[]
	}
