...
Log.defaultLogLevel = process.env.MANTLE_LOGLEVEL
	? (process.env.MANTLE_LOGLEVEL as LogLevel)
...
server.listen(PORT, () => {
	if (
		...
		log.error(
			'MQTT credentials are not set. Please set the MANTLE_MQTTUSERNAME and MANTLE_MQTTPASSWORD environment variables.'
		)
	} else {
