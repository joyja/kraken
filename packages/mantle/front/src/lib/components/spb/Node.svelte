<script lang="ts">
	...
	import type { SparkplugMetric } from '$lib/types'

	export let node
	export let group
	let expanded = false
	$: metrics = {
		commands: node.metrics.filter((metric: SparkplugMetric) => {
			return metric.id.includes('Node Control')
		}),
		info: node.metrics.filter((metric: SparkplugMetric) => {
			return metric.id.includes('Node Info')
		}),
		templates: node.metrics.filter((metric: SparkplugMetric) => {
			return (
				metric.value !== null &&
		...
		data: node.metrics.filter((metric: SparkplugMetric) => {
			return (
