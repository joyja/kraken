...
const getVariableValues = (varValues: any) =>
	Effect.tryPromise(() => {
		return sendRequest({
			query: query.variables
		}).then((res) => {
			const result = res.data?.variables.map(
				(variable: {
					datatype: string
					name: string
					children: { name: string; value: { path: string } }[]
				}) => {
					...
					if (atomicTypes.includes(variable.datatype)) {
						...
						const children = variable.children.map(
							(child: { name: string; value: { path: string }; datatype?: string }) => {
								const value = varValues.find(
									(value: { path: string }) => value.path === `${variable.name}.${child.name}`
								)?.value
								return {
									...child,
									value
						...
						const values = varValues
							...
							.map((value: { path: string; datatype: string; value: string }) => {
								const child = children.find(
									(child: { name: string }) =>
										child.name === value.path.split('.').slice(1).join('.')
								)
								return {
...
let variables: Promise<{ name: string; value: string }[]> = Effect.runPromise(
	retryGetVariables
).then((res) => {
