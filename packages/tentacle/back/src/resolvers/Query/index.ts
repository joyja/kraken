...
export function configuration():Config {
  return {
      tasks: Object.keys(plc.config.tasks).map((key) => {
        return {
          name: key,
          description: plc?.config?.tasks[key]?.description !== null && plc?.config?.tasks[key]?.description !== undefined
      ...
      mqtt: Object.keys(plc.config.mqtt).map((key) => {
        return {
          name: key,
          description: plc?.config?.mqtt[key]?.description !== null && plc?.config?.mqtt[key]?.description !== undefined
      ...
      modbus: Object.keys(plc.config.modbus).map((key) => {
        return {
          name: key,
          description: plc.config.modbus[key].description !== null && plc?.config?.modbus[key]?.description !== undefined
      ...
      opcua: Object.keys(plc.config.opcua).map((key) => {
        return {
          name: key,
          description: plc.config.opcua[key].description !== null && plc?.config?.opcua[key]?.description !== undefined
...
export function value(root:unknown, args:{variablePath:string}):{path:string, value:string, datatype:string} {
...
export function values():Array<{path:string, value:string, datatype:string}> {
...
export function variables ():Array<{name:string, description:string, datatype:string, children:Array<{name:string, description:string, datatype:string}>}> {
  return Object.keys(plc.variables).map((key) => {
    ...
    return {
      name: key,
      description: plc?.variables[key]?.description !== null && plc?.variables[key]?.description !== undefined
