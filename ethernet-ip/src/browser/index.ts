...
export declare interface Browser extends EventEmitter {
  ...
  on: ((event: 'New Device', listener: (device: browserDevice) => {}) => this) &
    ((event: 'Broadcast Request', listener: () => {}) => this) &
    ((
      event: 'Device Disconnected',
      listener: (device: browserDevice) => {},
    ) => this) &
    ((
      event: 'Device List Updated',
      listener: (deviceList: browserDevice[]) => {},
    ) => this) &
...
export class Browser extends EventEmitter {
  ...
  _setupSocketEvents() {
    this.socket.on('message', (msg) => {
      ...
      if (Object.keys(device).length !== 0) this._addDevice(device) // Device is added only if device is not empty object
    })
  }

  _parseListIdentityResponse(msg) {
    const response: browserDevice = {
  ...
  _addDevice(device: any): void {
    const index = this.deviceList.findIndex(
      (item) => item.socketAddress.sin_addr === device.socketAddress.sin_addr,
    )
